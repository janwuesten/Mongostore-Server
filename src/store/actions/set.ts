...
async function action (handler: MongoStoreHandler, query: core.Query, auth: null, req: core.Request, res: core.Response): Promise<MongoStoreResponse> { 
    ...
    if(searchForId) {
        response = await handler.set(query.collection as string, query.document as string, afterData, null)
    }else{
        response = await handler.set(query.collection as string, JSON.parse(query.query as string), afterData, null)
    }
    return response
}
